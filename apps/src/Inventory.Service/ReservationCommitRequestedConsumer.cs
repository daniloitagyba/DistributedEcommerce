...
public sealed class ReservationCommitRequestedConsumer(
    ...
    ILogger<ReservationCommitRequestedConsumer> logger) : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        var config = new ConsumerConfig
        {
            ...
            ClientId = $"{_options.ClientId}-commit",
        ...
        consumer.Subscribe(_options.CommitRequestedTopic);
        InventoryLog.Started(logger, _options.CommitRequestedTopic, _options.ConsumerGroup);
        ...
        CancellationToken cancellationToken)
    {
        for (var attempt = 1; attempt <= _processingOptions.MaximumAttempts; attempt++)
        {
            try
            {
                await processor.ProcessCommitAsync(consumeResult, cancellationToken);
                return true;
            }
