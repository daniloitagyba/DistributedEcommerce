...
public sealed class ReservationReleaseRequestedConsumer(
    ...
    ILogger<ReservationReleaseRequestedConsumer> logger) : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        var config = new ConsumerConfig
        {
            ...
            ClientId = $"{_options.ClientId}-release",
        ...
        InventoryLog.Started(logger, _options.ReleaseRequestedTopic, _options.ConsumerGroup);
        ...
        CancellationToken cancellationToken)
    {
        for (var attempt = 1; attempt <= _processingOptions.MaximumAttempts; attempt++)
        {
            try
            {
                await processor.ProcessReleaseAsync(consumeResult, cancellationToken);
                return true;
            }
