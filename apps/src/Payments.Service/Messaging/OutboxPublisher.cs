    ...
    ILogger<OutboxPublisher> logger) : BackgroundService
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            if (!string.Equals(message.EventType, nameof(PaymentDecided), StringComparison.Ordinal))
            {
                throw new JsonException($"Unsupported outbox event type '{message.EventType}'.");
            }

            paymentDecided = JsonSerializer.Deserialize<PaymentDecided>(message.Payload, SerializerOptions)
            ...
            await publisher.PublishAsync(paymentDecided, cancellationToken);
            message.MarkPublished(DateTimeOffset.UtcNow);
            OrdersTelemetry.RecordOutboxPublished(message.EventType);
            OutboxPublisherLog.Published(logger, message.Id, paymentDecided.OrderId, _instanceId);
        }
        catch (Exception exception) when (exception is not OperationCanceledException)
        {
...
{
    [LoggerMessage(EventId = 4000, Level = LogLevel.Information, Message = "Outbox publisher started on instance {InstanceId}")]
    public static partial void Started(ILogger logger, string instanceId);

    [LoggerMessage(EventId = 4001, Level = LogLevel.Information, Message = "Published outbox event {EventId} for order {OrderId} on instance {InstanceId}")]
    public static partial void Published(ILogger logger, Guid eventId, Guid orderId, string instanceId);

    [LoggerMessage(EventId = 4002, Level = LogLevel.Warning, Message = "Outbox event {EventId} failed on attempt {AttemptCount}; retry at {NextAttemptAt} on instance {InstanceId}")]
    ...
    [LoggerMessage(EventId = 4003, Level = LogLevel.Error, Message = "Outbox polling failed on instance {InstanceId}")]
    public static partial void LoopFailed(ILogger logger, string instanceId, Exception exception);

    [LoggerMessage(EventId = 4004, Level = LogLevel.Information, Message = "Outbox publisher is stopping gracefully on instance {InstanceId}")]
