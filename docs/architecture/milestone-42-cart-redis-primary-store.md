...
## Found by CI: the wrong Redis timeout for a store with no fallback

The first push failed CI - not a build error, a real design bug caught before it reached the cluster. `CartStore` initially reused BuildingBlocks' shared `"redis"` resilience pipeline (the same one `RedisOrderCache` uses), which has a **150ms** timeout. That number is correct for a cache: `RedisOrderCache` catches the resulting `TimeoutRejectedException` and falls back to querying Postgres directly, so failing fast is exactly right - the user gets a slightly slower response, not an error. Cart.Service has no such fallback; Redis *is* the store. Against a cold Testcontainers Redis on a loaded GitHub Actions runner, the very first write exceeded 150ms and the operation was cancelled outright, failing a request that would have succeeded with a couple hundred more milliseconds of patience.

Fixed with a dedicated `"cart-redis"` pipeline (`CartResilienceExtensions.AddCartRedisResilience`, 2-second timeout, same retry/circuit-breaker shape as the Postgres pipeline) - shared between `Program.cs` and the integration tests via one extension method rather than duplicated. The lesson generalizes: a resilience policy's "right" timeout depends on what happens when it fires, not just on how fast the underlying call normally is. A shared pipeline tuned for a fallback-having consumer is a latent bug waiting for the first fallback-less consumer to reuse it.
