...
The existing `test` job's `dotnet test` step now runs with `--collect:"XPlat Code Coverage"`, followed by `reportgenerator` producing an HTML + text summary artifact, followed by a `grep`+`awk` gate comparing the reported line coverage against a threshold.

**Real measured baseline: 43.7% line coverage.** Threshold set to **35%** - a few points under the baseline so ordinary run-to-run variance doesn't trip it, gating further *regression* rather than demanding the whole solution's untested infrastructure/glue code jump to a high bar overnight.
...
| Coverage threshold | coverlet + reportgenerator | 35% (baseline 43.7%) | - |
...
# Coverage
dotnet test apps/DistributedEcommerce.slnx --collect:"XPlat Code Coverage" --results-directory ./coverage
