...
Deliberately stops there. Application manifests (`kubernetes/base`, `kubernetes/overlays/local`) are Argo CD's job (Milestone 15), not this playbook's - running this against a fresh host still needs `scripts/k3s-deploy.sh` and the Argo CD bootstrap `Application` afterward, per the main README's "Deploy the applications to K3s" section.
...
- **Secrets** - `compose/.env`, the sealed-secrets private key, and every credential this lab generates locally are deliberately outside version control and outside this playbook. Provisioning them is `scripts/k3s-deploy.sh`'s job (streams the Orders DB connection string into a K8s Secret without printing or storing it) and each milestone's own setup script (`scripts/keycloak-configure-realm.sh`, `scripts/postgres-ha-provision.sh`).
